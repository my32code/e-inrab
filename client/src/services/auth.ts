...
export const login = async (credentials: {
  ...
  const response = await fetch(`${import.meta.env.VITE_BACKEND_HOST}/api/auth/login`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export const logout = async () => {
  try {
    ...
    if (sessionId) {
    await fetch(`${import.meta.env.VITE_BACKEND_HOST}/api/auth/logout`, {
      method: 'POST',
        headers: {
...
export const isAuthenticated = async (): Promise<boolean> => {
  try {
    ...
    const response = await fetch(`${import.meta.env.VITE_BACKEND_HOST}/api/auth/verify`, {
      headers: {
        'Authorization': `Bearer ${sessionId}`
      }
    });

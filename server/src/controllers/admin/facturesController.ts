...
const generateFactureHTML = (data: any) => {
  ...
  return `
    ...
    <body>
      <div class="header">
        <img src="/logo.png" alt="Logo INRAB" class="logo">
      ...
      <div class="valid-until">
        <p>Valable jusqu'au: ${format(validUntil, 'dd MMMM yyyy', { locale: fr })}</p>
...
export const generateFacture = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const templateData = {
      ...
      items: [{
        ...
        prix_unitaire: type === 'commande' ? parseFloat(data.prix_unitaire) : parseFloat(data.prix)
      }],
      total: type === 'commande' 
